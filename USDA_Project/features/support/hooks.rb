...
After do |scenario|
  if scenario.failed?
    if ENV['Screen_Shot']=="yes"
      Dir::mkdir('Screen_Shots') if not File.directory?('Screen_Shots')
      screenshot = "./Screen_Shots/FAILED_#{scenario.name.gsub(' ', '_').gsub(/[^0-9A-Za-z_]/, '')}.png"
...
at_exit do
  ...
  if ENV['ARCHIVE_RESULTS']=="yes"
    ...
    test_result="USDA Test Execution Result"
    ...
    Zip::File.open(zipfile_name, Zip::File::CREATE) do |zipfile|
      input_filenames.each do |filename|
        ...
        if ENV['Email_Test_Results']=="yes"
          ...
          message.Recipients.Add 'quazihrashid@gmail.com'
          # message.Recipients.Add 'abusayed.m.mahfuz@uscis.dhs.gov'
